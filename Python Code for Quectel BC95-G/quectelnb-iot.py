...
remote_port = "5001"
remote_ip = "134.209.201.250"

#remote_port = "5683"
#remote_ip = "52.17.209.228"

apname = os.getenv("apn", "company.iot.dk1.tdc")
NBAND = 20
solicited = "1"
# Number used to register with a network operator
network_operator = os.getenv("network_operator", "23801")

# Serial port of device. e.g. com4 on Windows, /dev/ttyACM0 | /dev/ttyUSB0 on Linux
serial_name = os.getenv("serial_name", "com19")
info=[]

#acces token is your Device secret key found in the Wia dashboard
data = {"accessToken": os.getenv("accessToken", "d_sk_e0ThsopEdqPfbmSTMrjYQrS7"), "name": "QUECTEL", "data": "testing"}
...
-e : network stastistics (AT+NUESTATS)"
    try:
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            print "Setting module configure"
            serialport.write("AT+NCONFIG=\"CR_0859_SI_AVOID\",\"FALSE\"\r")
            time.sleep(5)
            response = serialport.readlines(None)
            if "OK" not in str(response):
                print str(response).replace('\r\n', '')
            ...
            print "Enable the Power Saving Mode"
            serialport.write("AT+NPSMR=1\r")
            time.sleep(5)
            response = serialport.readlines(None)
            if "OK" not in str(response):
                print str(response).replace('\r\n', '')
            ...
            #serialport.write("AT+COPS=1,2,\"23801\",7\r")
            serialport.write("AT+COPS=1,2,\"23801\"\r")
            response = serialport.readlines(None)
            if "OK" in str(response):
                print bcolours.ok()
            else:
            ...
            while attempts:
                ...
                cid= str(response[1].replace('\r\n', ''))
                cid1= cid.replace('+CEREG:', '')
                cid2= cid1.replace('2,', '')
                cid3= cid2.replace('1,', '')
                cid4=cid3.replace('26AC,','')
                cid5=cid4.replace(',9','')
                cid6=cid5.replace('"','')
                info.append(cid6)
                print cid6

                if '+CEREG:2,1' in str(response):
                    break
                attempts -= 1
                if attempts == 0:
            ...
            response = serialport.readlines(None)
            csq = str (response[1].replace('\r\n', ''))
            csq1 = csq.replace('+CSQ:', '')
            info.append(csq1)
            str1 = ' '.join(info)

            print str1
            print str(response[1].replace('\r\n', '')) 


        elif opt in ("-e",):
            print "Checking Network stastistics"
            serialport.write("AT+NUESTATS\r")
            response = serialport.readlines(None)
            print str(response).replace('\r\n', '') , "\n" 
            status=str(response).replace('\r\n', '')
            info.append(status)
            str1= ' '.join(info)
            ...
            print bcolours.BOLD,"Sending The device is in PSM", bcolours.BOLD
            serialport.write("AT+CPSMS=1,,,\"00100001\",\"00000000\"\r")
            response = serialport.readlines(None)
            if "OK" in response[1]:
                print "Power saving mode is Initiated"
            else:
                print "Power saving mode is not Initiated"        
