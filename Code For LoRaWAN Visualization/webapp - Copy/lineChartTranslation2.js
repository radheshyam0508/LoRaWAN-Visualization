function LoadLineChart(elementId, updateData, min, max, samprate, dataCount,useId) {
	...
	//d3 defined the color as black, delete it at first.
	...
	svg.selectAll(".path") //select all the <path> in <svg>
	.data(updateData) //bland the data
	.transition()
	.duration(500)
	.ease(d3.easeLinear)
	.attr("d", function (d) {

		return linePath(d.alldatas); //return to the path generater
	});
	...
	this.updateLineChart = function () {
		...
		transition = transition.each(function () {
				...
				.on("mouseover", function (d) {
					...
					tooltip.html("Time: " + timeFormat(new Date(d[0])) + "<br />" +
						"Temperature: " + d[1])
